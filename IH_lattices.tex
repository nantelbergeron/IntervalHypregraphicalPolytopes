...
We are not sure whether the backward direction holds in greater generality.
\end{remark}
...
Assume first that~$\II$ is is not closed under initial subintervals, and let~$1 \le i < j < k \le n$ be such that~$[i,j] \notin \II$ while~$[i,k] \in \II$.
\vincent{TODO}
