...
\begin{equation}\label{eq:orientation}
	\Or_\pi = \Big\{\big(a,H\big)\big| H\in \HH, |H|>1,  a=\pi\big(\min\{j: \pi(j)\in H\}\big)\Big\}\,.
\end{equation}
...
the order $\less_{(a,H)}=\big\{ a< b : b\in H\ssm\{a\}\big\}$ for each $(a,H)\in \Or$ . That is
 	$$\less_\Or =  \bigcup_{(a,H)\in \Or} 
	\begin{tikzpicture}[scale=1,baseline=.0cm]
	\node at (0,-.4) {$\scriptstyle a$};
	\node at (0,.6) {$\scriptstyle b\in H\ssm \{a\}$};
	\node at (.2,.35) {$\ldots$};
	\draw [thick] (0,-.3)--(-.5,.4); 
	\draw [thick] (0,-.3)--(-.3,.4); 
	\draw [thick] (0,-.3)--(-.1,.4); 
...
Let $i<j<k$ be any integers in the natural order of $[n]$. If we have $i\less_{\Or} k$, then, by definition of $\less_\Or$, there must be a sequence $(a_1,I_{1}), (a_2,I_{2}),\ldots (a_m,I_{m})$ of oriented edges in $\Or$ such that
$a_1=i$, for $1\le s<m$ we have $a_{s+1}\in I_s$, and $k\in I_m$. We must therefor have $j\in\bigcup_{1\le s\le m} I_s$ since both $i$ and $k$ are in the union. This implies $i\less_{\Or} j$. The case $i\more_{\Or} k$ is similar and  implies
