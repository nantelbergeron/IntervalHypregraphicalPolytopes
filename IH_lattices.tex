...
for all~$[a,a'], [b,b'], [c,c'], [d,d'] \in \II$ such that $a < b \le a' < b'$, $a < c \le b' < c'$, $d < \min(b, c)$ and~$c' \le d'$, there is~$[e,e'] \in \II$ such that~$e < \min(b, c)$ and~$b' \le e' < c'$.
...
We say that an interval hypergraph~$\II$ is \defn{join semidistributive} if it is closed under intersection and for all~$[a,a'], [b,b'], [c,c'], [d,d'] \in \II$ such that $a < b \le a' < b'$, $a < c \le b' < c'$, $d < \min(b, c)$ and~$c' \le d'$, there is~$[e,e'] \in \II$ such that~$e < \min(b, c)$ and~$b' \le e' < c'$.
%        if p0[0] < p1[0] <= p0[1] < p1[1] and p0[0] < p2[0] <= p1[1] < p2[1] and p1[1] != p2[1] and all(inter[0] >= min(p1[0], p2[0]) or inter[1] < p1[1] or inter[1] >= p2[1] for inter in intervals) and any(inter[0] < min(p1[0], p2[0]) and p1[1] < inter[1] for inter in intervals):
%            return False
\end{definition}
...
A dual characterization holds for meet semidistributive.
\end{theoremA}
...
\begin{notation}
For $(i,j), (k,\ell)$, we write $(i,j) \preccurlyeq (k,\ell)$ if and only $j \le \ell$ and~$\ell \in J_{ij}$ and~$\mu_{ij} \ge \mu_{k\ell}$.
\vincent{not sure we really need this definition...}
\end{notation}
...
\end{gather*}
and we have~$A_{23} \le A_{13}$ and~$A_{34} \le A_{24} \le A_{14}$.
\end{example}
...
\end{gather*}
and we have~$A_{12} \le A_{24}$ and~$A_{34} \le A_{24}$.
\end{example}
...
\end{gather*}
and we have~$A_{23} \le A_{13}$ and~$A_{34} \le A_{14}$.
\end{example}
...
\end{gather*}
and we have~$A_{23} \le A_{13}$ and~$A_{24} \le A_{14}$.
\end{example}
...
As $j = \ell$ and $\mu_{ij} = \mu_{k\ell}$, we obtain that~$i = k$ by definition of~$\IJ_\II$
...
\vincent{not sure we really need this lemma...}
\end{lemma}
...
Assume that there is~$[a,a'], [b,b'], [c,c'], [d,d'] \in \II$ such that $a < b \le a' < b'$, $a < c \le b' < c'$, $d < \min(b, c)$ and~$c' \le d'$, and there is no~$[e,e'] \in \II$ such that~$e < \min(b, c)$ and~$b' \le e' < c'$.
Let
\[
j \eqdef a'+1,
\quad
i \eqdef \max \Big( [c,j[ \ssm \!\!\!\!\! \bigcup_{\substack{M \in \II \\ M \subseteq [c,j[}} \!\!\!\!\! M \ssm \{\min(M)\} \Big)
\;\;\text{and}\;\;
i' \eqdef \max \Big( [d,j[ \ssm \!\!\!\!\! \bigcup_{\substack{M \in \II \\ M \subseteq [d,j[}} \!\!\!\!\! M \ssm \{\min(M)\} \Big).
\]
Similarly, let
\[
\ell \eqdef b'+1
\qquad
k \eqdef \max \Big( [d,\ell[ \ssm \!\!\!\!\! \bigcup_{\substack{M \in \II \\ M \subseteq [d,\ell[}} \!\!\!\!\! M \ssm \{\min(M)\} \Big)
\quad\text{and}\quad
k' \eqdef \max \Big( [d,\ell[ \ssm \!\!\!\!\! \bigcup_{\substack{M \in \II \\ M \subseteq [d,\ell[}} \!\!\!\!\! M \ssm \{\min(M)\} \Big).
\]
Note that~$i = k$.
Consider the join irreducible acyclic orientations~$A_{ij}, A_{i'j}, A_{k\ell}$ and~$A_{k'\ell}$ as defined in \cref{not:joinIrreducibles}.
Moreover, let~$A$ be defined as ...
\vincent{todo: define properly~$A$.}
Observe that
\[
A_{ij} \join A_{i'j} = A = A_{k\ell} \join A_{k'\ell}.
\]
Moreover
\[
A_{ij} < A_{i'j}
...
A_{k\ell} < A_{k'\ell},
\]
and there is no other comparisons among~$A_{ij}, A_{i'j}, A_{k\ell}$ and~$A_{k'\ell}$.
Hence, $A$ admits two incomparable join representations, so that~$P_\II$ is not join irreducible.
\vincent{todo: prove all statements.}
\end{proof}
...
a & a' & b & b' & c & c' & d & d' & i & i' & j & k & k' & \ell \\
\hline
1 & 2 & 2 & 3 & 2 & 4 & 1 & 4 & 2 & 1 & 3 & 2 & 1 & 4
\end{array}
\]
and
\[
\begin{array}{c@{}c@{}c@{}c@{}c@{}c@{}c}
A_{23} && A_{14} & A & A_{13} && A_{24} \\
...
\end{array}
\]
and we have~$A_{23} \le A_{13}$ and~$A_{24} \le A_{14}$ but no other comparisons, so that~$A$ admits two incomparable join representations.
\end{example}
