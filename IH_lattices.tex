...
\begin{figure}[b]
	\centerline{
		\begin{tabular}[t]{c}
			\input{figures/hypergraphicalPolytope} \\[.1cm]
...
If there is~$J \in \II$ such that~$B(J) \notin \{\min(J), j\}$, then \cref{lem:alwaysFlippable} ensures that $B$ admits a decreasing flip flipping~$A(J')$ to some~$k < A(J') \ne j$.
...
Hence, \cref{lem:alwaysFlippable} ensures that $B$ admits a decreasing flip flipping~$j$ to some~$k < \mu_j$.
...
\begin{proof}
Consider a join irreducible acyclic orientation~$B$ of~$\II$ and let~$\flip{A}{i}{j}{B}$ be the only increasing flip to~$B$.
