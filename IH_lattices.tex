...
\Nantel{put here the condition that it is enough to test cyclicity between any two segment}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{$\less_\Or$ is separated}  
\label{subsec:sourceinc}

\

\Nantel{remark that $\less_\Or$ is separated}
...
$i=i_1<i_2<\cdots <i_{m}$, for $1\le s<m$ we have $i_{s+1}\in I_s$, and $k\in I_m$. We must therefor have $j\in\bigcup_{1\le s\le m} I_s$ since both $i$ and $k$ are in the union. This implies $i\less_{\Or} j$. The case $i\more_{\Or} k$ is similar and  implies
...
Let $\pi$ be such that $A=\Or_\pi$. 
%Fix $K\in \II$ be a segment such that $a=S(K,A)$ and $b=S(K,B)$.
%and in particular $[a,b]\subseteq K$.
We claim that for $\pi'=(a,b)\pi$ the orientation $A'=\Or_{\pi'}$ satisfies $A<A'$ and for all $I\in \II$ we have $S(I,A')\le S(I,B)$.
our result would then follow by induction.

To show the claim, consider any $I\in \II$. If $s(I,A)=a$ and $b\in I$ then we must have $S(I,A')=b$. 
%In particular $[a,b]\subseteq I$.
We also have $b=S(I,A')\le S(I,B)$ since $S(I,B)>b$ would contradict the maximality of the choice of $b$.
In all other case $S(I,A')=S(I,A)\le S(I,B)$. Finally we need to show that $A<A'$. 
\Nantel{I don't understand the argument of Vincent in the note. What guaranties that flipping $a$ and $b$ gives comparable elements?
I priori, even if we show it is a valid orientation, it may give us something incomparable? We have to show it is obtained by a (sequence) of weak order relations.
I think I will have to describe cover first... I'll come back to this later.}
\end{proof}
...
Thus we only need to prove (b)$\implies$(d). For contradiction, assume we have some $i<j$ such that $j\,\less_Ai$ and  $i \,\less_B j$. Choose one such pair such that $|j-i|$ is minimal.
Since $j\,\less_Ai$, 
$(j_1,I_{1}), (j_2,I_{2}),\ldots (i_m,I_{m})$ such that $j=j_1>j_2>\cdots>j_m$, each $j_{s+1}\in I_s$ and $i\in I_m$.
\Nantel{to be continued}
