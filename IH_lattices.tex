...
See \cref{fig:notLattices,fig:Tamari,fig:distributiveLattices,fig:semidistributiveLattices,fig:notSemidistributiveLattices} for illustrations of these characterizations.
...
For instance, $(2,3)\, \begin{tikzpicture}[scale=1,baseline=-.1cm] \path[color=red, ->]  (0,0) edge node[fill=white,inner sep=2pt, font=\tiny ] {$  23$} (1,0); \end{tikzpicture}\, (3,3)$ indicates that there is an increasing flip from  $O=(2,3)$ to $O'=(3,3)$ with $i=2<3=j$.
\end{example}
...
\begin{example}
We have represented some hypergraphic posets in \cref{fig:notLattices,fig:Tamari,fig:distributiveLattices,fig:semidistributiveLattices,fig:notSemidistributiveLattices}.
\vincent{list of examples from the introduction...}
\end{example}
...
\begin{theoremA}
For an interval hypergraph $\II$ on~$[n]$ with~$\{i\} \in \II$ for all~$i \in [n]$, the poset $P_\II$ is a semidistributive lattice if and only if $\II$ is semidistributive.
\end{theoremA}
