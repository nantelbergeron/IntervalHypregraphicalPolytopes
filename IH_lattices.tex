...
\begin{remark}
\cref{prop:preimageI} fails for arbitrary hypergraphs~$\HH$, for instance for~$\HH \eqdef \{1,2,3,13\}$,\nantel{ Just $\scriptstyle \{ 13\}$ if we remove singleton} there are two fibers~$\{123, 213, 132\}$ and~$\{231, 312, 321\}$ which are not intervals of the weak order.
\end{remark}
...
\begin{proof}
The forward direction is immediate\nantel{ as it holds for increasing flip by \cref{def:flip} and any cover is an increasing flip.} as it holds for cover relations of~$P_\II$ by \cref{def:flip}.
...
Then $x = A_x(J_x) \le \big( \bigJoin\limits_{x \in X} A_x \big)(J_i) = \big( \bigJoin\limits_{y \in Y} A_y \big)(J_i)$.
From the description of the join of \cref{prop:joinLattice}, we thus obtain that there exists~$y \in Y$ and~$J \in \II$ such that~$A_y(J) \in I$ and~$\min(J) \le x < A_y(J)$.
\vincent{IN PROGRESS...}
\end{proof}
