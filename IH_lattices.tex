...
If $\delta_{A,B}=\emptyset$, then we have $A=B$ and the result holds. Otherwise, choose $(a,b)\in \delta_{A,B}$ such that $b$ is maximal and let $\pi$ be such that $A=\Or_\pi$. 
...
For the  permutation $\pi$, let $i<k$ be such that $\pi(i)=a$ and $\pi(k)=b$. Assume there is $i<j<k$ such that 
\Nantel{here I don't see the argument on Vincent... trying something else and I don't remember what I did. I'll rest on it for now}
