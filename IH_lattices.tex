	...
	\centerline{
		\begin{tabular}{c@{\qquad}c}
			\begin{tikzpicture}[scale=1.5,baseline=-3.5cm]
				...
				\node (d) at (0,2.7) {\acyclicOrientation{3}{1/2/2,2/3/3,1/3/3}};
...
\begin{example}
The interval hypergraphic posets of \cref{fig:notLattices} are not lattices, while those of \cref{fig:Tamari,fig:distributiveLattices,fig:notSemidistributiveLattices} are lattices.
%Note that
%\begin{itemize}
...
\begin{example}
The interval hypergraphic posets of \cref{fig:distributiveLattices} are distributive lattices, while those of \cref{fig:notLattices,fig:Tamari,fig:notSemidistributiveLattices} are not.
%Note that,
%\begin{itemize}
...
By definition~$\mu_{ij} < i < j \le \nu_{ij}$ since~${i,j} \subseteq J_j \ssm \{\mu_j\}$.
\end{notation}

\begin{notation}
For $(i,j), (k,\ell) \in \cIJ_\II$, we write $(i,j) \preccurlyeq (k,\ell)$ if and only if $i \ge k$ and~$j = \ell$.
\vincent{Is this correct? It seems wrong...}
\end{notation}
