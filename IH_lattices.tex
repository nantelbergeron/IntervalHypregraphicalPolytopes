...
\end{itemize}
In fact, it follows from~\cite{PadrolPaluPilaudPlamondon} that the interval hypergraphic polytopes are precisely the weak Minkowski summands of the classical associahedron (recall that a polytope~$P \subset \R^n$ is a \defn{weak Minkowski summand} of a polytope~$Q \subset \R^n$ if there exists a real~$\lambda \ge 0$ and a polytope~$R \subset \R^d$ such that~$\lambda Q = P + R$).
...
For an interval hypergraph $\II$, the projection map from the weak order to $P_\II$ is a meet (resp.~join) semilattice morphism if and only if~$\II$ is closed under initial (resp.~final) subintervals (\ie,~$[i,k] \in \II$ implies~$[i,j] \in \II$ (resp.~$[j,k] \in \II$) for any~$1 \le i < j < k \le n$).
\end{theoremA}
...
In \cref{sec:HP}, we recall basic properties of hypergraphic polytopes, we define hypergraphic posets, and we recall the natural poset morphism from the weak order on permutations to the hypergraphic poset.
In \cref{sec:IHP}, we develop specific properties of interval hypergraphic polytopes, in particular a simple characterization of their vertices and a simple description of the relations in their hypergraphic posets.
...
In \cref{sec:distributive}, we describe a family of join irreducible elements of~$P_\II$ and we characterize the interval hypergraphs~$\II$ for which the interval hypergraphic poset~$P_\II$ is a distributive lattice, proving \cref{thm:distributiveLatticeI}.
In \cref{sec:semidistributive}, we describe all join irreducible elements of~$P_\II$ and we characterize the interval hypergraphs~$\II$ for which the interval hypergraphic poset~$P_\II$ is a lattice, proving \cref{thm:semidistributiveLatticeI}.
In \cref{sec:quotient}, we characterize the interval hypergraphs~$\II$ for which the interval hypergraphic poset~$P_\II$ is a lattice, proving \cref{thm:quotientLatticeI}.
...
\[
	...
	\end{tikzpicture}
\]
and the left edge of $\simplex_\HH$ is not a cover relation of~$P_\HH$.
\end{remark}
...
As~$\simplex_\HH$ is a deformed permutahedra (\aka generalized permutahedra~\cite{Postnikov, PostnikovReinerWilliams}, \aka polymatroid~\cite{Edmonds}), there is a natural surjection from the faces of the permutahedron to the faces of~$\simplex_\HH$, which was described in details in~\cite[Lem.~2.9]{BenedettiBergeronMachacek}.
...
\begin{figure}
	...
	\caption{The poset morphism~$\Or$ from the weak order (left) to the hypergraphical poset~$P_\HH$ (right), for $\HH=\{ 1, 2, 3, 4, 123, 134 \}$. The fibers of~$\Or$ appear as blue bubbles on the weak order (left).}
...
For instance, 
$\II = \{ 1, 2, 3, 4,123, 23, 234, 1234 \}$ 
is an interval hypergraph and
%$\Or_{4132} = \big\{ (4, \{1,2,3,4\}), (1, \{1,2,3\}), (3, \{2,3\}), (4, \{2,3,4\})\big\}$ 
$\Or_{4132} = (1, 3, 4, 4)$ is an acyclic orientation of~$\II$ (for the order $(123, 23, 234, 1234)$ of the non-singletons).
...
\begin{figure}
	\centerline{
		\begin{tabular}[t]{c}
			\input{figures/hypergraphicalPolytope} \\[.5cm]
			\includegraphics[scale=.7]{weakOrderCongruence2}
		\end{tabular}
		\raisebox{-6.2cm}{
			\begin{tikzpicture}[scale=2.5]
				\node (a) at (3,0) {\acyclicOrientation{4}{1/3/1,1/4/1,2/3/2,2/4/2}};
				\node (b) at (1,1) {\acyclicOrientation{4}{1/3/2,1/4/2,2/3/2,2/4/2}};
				\node (c) at (3,1) {\acyclicOrientation{4}{1/3/1,1/4/1,2/3/3,2/4/3}};
				\node (d) at (4,1) {\acyclicOrientation{4}{1/3/1,1/4/1,2/3/2,2/4/4}};
				\node (e) at (1,2) {\acyclicOrientation{4}{1/3/3,1/4/3,2/3/3,2/4/3}};
				\node (f) at (3,2) {\acyclicOrientation{4}{1/3/1,1/4/4,2/3/2,2/4/4}};
				\node (g) at (4,2) {\acyclicOrientation{4}{1/3/1,1/4/1,2/3/3,2/4/4}};
				\node (h) at (2,3) {\acyclicOrientation{4}{1/3/2,1/4/4,2/3/2,2/4/4}};
				\node (i) at (3,3) {\acyclicOrientation{4}{1/3/1,1/4/4,2/3/3,2/4/4}};
				\node (j) at (2,4) {\acyclicOrientation{4}{1/3/3,1/4/4,2/3/3,2/4/4}};
				\draw (a)--(b);
				\draw (a)--(c);
				\draw (a)--(d);
				\draw (b)--(e);
...
Consider an acyclic orientation~$A$ of an interval hypergraph~$\II$ such that there is~$I \in \II$ and~$i \in [n]$ such that~$i = A(I) < \max(I)$.
...
As~$j \in X$, there~$J \in \II$ such that~$A(J) \in {]i,k]}$ and~$j \in J \ssm \{A(J)\}$.
...
For instance, for the hypergraph $\HH=\{ 1, 2, 3, 4, 123, 134 \}$ of \cref{ex:DH1,ex:DH2,ex:DH3,ex:DH4,ex:DH5}, and the acyclic orientation~$A$ defined by~$A(123) = 2$ and~$A(134) = 1$, we have~$2 = A(123) < \max(123)$, but no increasing flip to an acyclic orientation of~$\HH$ flips~$2$.
\end{remark}
...
In this section, we prove \cref{thm:latticeI} which we first introduce properly: % characterizing the interval hypergraphs $\II$ for which $P_\II$ are lattices.
...
\begin{example}
For the hypergraph~$\{ 1, 2, 3, 4,123, 23, 234, 1234 \}$ of \cref{exm:intervalHypergraph,fig:exmInterval}, we have
\[
\raisebox{-.2cm}{\acyclicOrientation{4}{1/3/2,1/4/2,2/3/2,2/4/2}} \join \raisebox{-.2cm}{\acyclicOrientation{4}{1/3/1,1/4/1,2/3/3,2/4/3}} = \Or_{2134 \join 1324} = \Or_{3214} = \raisebox{-.2cm}{\acyclicOrientation{4}{1/3/3,1/4/3,2/3/3,2/4/3}}
\]
and
\[
\raisebox{-.2cm}{\acyclicOrientation{4}{1/3/2,1/4/2,2/3/2,2/4/2}} \meet \raisebox{-.2cm}{\acyclicOrientation{4}{1/3/1,1/4/1,2/3/3,2/4/3}} = \Or_{2431 \meet 1342} = \Or_{1234} = \raisebox{-.2cm}{\acyclicOrientation{4}{1/3/1,1/4/1,2/3/2,2/4/2}}
\]
\end{example}
...
Our first task will be to identify some join irreducible acyclic orientations of~$\II$ (a complete but more technical description of the join irreducible acyclic orientations of~$\II$ will appear later in~\cref{subsec:joinIrreducibles}).
...
\begin{example}
\label{exm:Tamari1}
For the hypergraph~$\II = \{1, 2, 3, 12, 23, 123\}$  illustrated in \cref{fig:Tamari}\,(left), we have~$\cJ_\II = \{2,3\}$ and the corresponding acyclic orientations are
\[
...
\begin{example}
\label{exm:Tamari2}
For the hypergraph~$\II \eqdef \{1, 2, 3, 12, 23, 34, 123, 234, 1234\}$ illustrated in \cref{fig:Tamari}\,(right), we have~$\cJ_\II = \{2,3,4\}$, and the corresponding acyclic orientations~are
\[
A_2 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/2/2,2/3/2,3/4/3,1/3/2,2/4/2,1/4/2}}
\qquad
A_3 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/2/1,2/3/3,3/4/3,1/3/1,2/4/3,1/4/1}}
\quad\text{and}\quad
A_4 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/2/1,2/3/2,3/4/4,1/3/1,2/4/2,1/4/1}}
\]
and are incomparable.
\end{example}

\begin{example}
For the hypergraph~$\II \eqdef \{1,2,3,4,123,23,234\}$ illustrated in \cref{fig:distributiveLattices}\,(top left), we have~$\cJ_\II = \{2,3,4\}$, and the corresponding acyclic orientations~are
\[
A_2 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/2,2/3/2,2/4/2}}
\qquad
A_3 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/1,2/3/3,2/4/3}}
\quad\text{and}\quad
A_4 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/1,2/3/2,2/4/4}}
\]
and are incomparable.
\end{example}

\begin{example}
For the hypergraph~$\II \eqdef \{1,2,3,4,123,1234\}$ illustrated in \cref{fig:distributiveLattices}\,(top right), we have~$\cJ_\II = \{2,3,4\}$, and the corresponding acyclic orientations~are
\[
A_2 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/2,1/4/2}}
\qquad
A_3 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/3,1/4/3}}
\quad\text{and}\quad
A_4 = \raisebox{-.4cm}{\acyclicOrientation{4}{1/3/1,1/4/4}}
\]
and we have~$A_2 < A_3$.
\end{example}
...
%Let~$A < B$ be two acyclic orientations of~$\II$ related by an increasing flip from~$i$ to~$j$.
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 12, 23, 123\}$ of \cref{exm:Tamari1,exm:oneMoreIrreducible,fig:Tamari}\,(left), we have~$\cIJ_\II = \{(1,2), (2,3), (1,3)\}$, and the join irreducible acyclic orientations~are %~$A_{12} = A_2$, $A_{23} = A_3$ and~$A_{13} = A$.
\[
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 12, 23, 34, 123, 234, 1234\}$ of \cref{exm:Tamari2,fig:Tamari}\,(right), we have~$\cIJ_\II = \{(1,2), (2,3), (3,4), (1,3), (2,4), (1,4)\}$, and the join irreducible acyclic orientations~are
\begin{gather*}
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 4, 123, 23, 234, 1234\}$ illustrated in \cref{fig:semidistributiveLattices}\,(left), we have~$\cIJ_\II = \{(1,2), (2,3), (2,4), (1,4)\}$, and the join irreducible acyclic orientations~are
\begin{gather*}
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 4, 23, 34, 1234\}$ illustrated in \cref{fig:semidistributiveLattices}\,(right), we have~$\cIJ_\II = \{(1,2), (2,3), (3,4), (2,4)\}$, and the join irreducible acyclic orientations~are
\begin{gather*}
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 4, 12, 23, 34, 1234\}$ illustrated in \cref{fig:notSemidistributiveLattices}\,(left), we have~$\cIJ_\II = \{(1,2), (2,3), (3,4), (1,3), (1,4)\}$, and the join irreducible acyclic orientations~are
\begin{gather*}
...
\begin{example}
For the hypergraph~$\II \eqdef \{1, 2, 3, 4, 12, 23, 234, 1234\}$ illustrated in \cref{fig:notSemidistributiveLattices}\,(right), we have~$\cIJ_\II = \{(1,2), (2,3), (3,4), (1,3), (1,4)\}$, and the join irreducible acyclic orientations~are
\begin{gather*}
...
\[
{]k,\ell[} \subseteq \bigcup_{\substack{J \in \II \\ A_{ij}(J) \in [k,\ell[}} J \ssm \{A_{ij}(J)\}. % = \bigcup_{\substack{J \in \II \\ A_{ij}(J) \in [k,j[}} J \ssm \{\min(J)\}
\]
...
%\begin{definition}
%A map~$\phi$ between two join semilattices~$(L, \le, \join)$ and~$(L', \le', \join')$ is a \defn{join semilattice morphism} if~$\phi(x \join y) = \phi(x) \join' \phi(y)$ for all~$x,y \in L$.
%Equivalently, if
%\begin{itemize}
%\item each fiber~$F$ of~$\phi$ is order convex (meaning that~$x < y < z$ and~$x,y \in F$ implies~$y \in F$) and admits a unique maximal element,
%\item the map~$\projUp : L \to L$ sending an element~$x$ to the maximal element~$y$ with~$\phi(x) = \phi(y)$ is order preserving.
%\end{itemize}
%The meet semilattice morphisms are defined and characterized dually.
%\end{definition}

\begin{definition}
A map~$\phi$ between two meet semilattices~$(L, \le, \meet)$ and~$(L', \le', \meet')$ is a \defn{meet semilattice morphism} if~$\phi(x \meet y) = \phi(x) \meet' \phi(y)$ for all~$x,y \in L$.
Equivalently, if
\begin{itemize}
\item each fiber~$F$ of~$\phi$ is order convex (meaning that~$x < y < z$ and~$x,y \in F$ implies~$y \in F$) and admits a unique minimal element,
\item the map~$\projDown : L \to L$ sending an element~$x$ to the minimal element~$y$ with~$\phi(x) = \phi(y)$ is order preserving.
\end{itemize}
The join semilattice morphisms are defined and characterized dually.
\end{definition}

\begin{theoremA}
For an interval hypergraph $\II$ on~$[n]$ with~$\{i\} \in \II$ for all~$i \in [n]$, the map~$\Or$ from the weak order on~$\fS_n$ to $P_\II$ is a meet (resp.~join) semilattice morphism if and only if $\II$ is closed under initial (resp.~final) subintervals.
\end{theoremA}

%\begin{proof}
%We only prove the join semilattice morphism, the result for meet semilattice morphism follows by the symmetry of~\cref{prop:antiIsomorphism}.
%
%Assume first that~$\II$ is is not closed under final subintervals, and let~$1 \le i < j < k \le n$ be such that~$[j,k] \notin \II$ while~$[i,k] \in \II$.
%
%
%Conversely, assume that~$\II$ is closed under final subintervals.
%We prove by induction on the number of non-inversions of~$\sigma$ that~$\sigma \le \tau$ implies~$\projUp(\sigma) \le \projUp(\tau)$.
%%Let~$\sigma \lessdot \tau$ be two adjacent permutations on~$[n]$.
%%We prove by induction on the number of non-inversions of~$\sigma$ that~$\projUp(\sigma) \le \projUp(\tau)$.
%We have nothing to prove if~$\sigma = \projUp(\sigma)$. % or~$\projUp(\sigma) = \projUp(\tau)$.
%If there is a permutation~$\rho$ such that~$\sigma < \rho < \tau$, then we have~$\projUp(\rho) \le \projUp(\tau)$ by induction, so that we just need to show that~$\projUp(\sigma) \le \projUp(\rho)$.
%We can thus assume that~$\sigma \lessdot \tau$ is a cover relation, with~$\sigma \ne \projUp(\sigma)$.
%Let~$\sigma \lessdot \sigma'$ be such that~$\Or_\sigma = \Or_{\sigma'}$.
%Let~$p,q \in [n-1]$ be such that~$\tau$ (resp.~$\sigma'$) is obtained from~$\sigma$ by exchanging its entries at positions~$p$ and~$p+1$ (resp.~$q$ and~$q+1$).
%We distinguish three cases:
%
%\para{If~$|p-q| > 1$.} Consider~$\tau' = \sigma' \join \tau$. As~$\Or_\sigma = \Or_{\sigma'}$, we have~$\min(\sigma^{-1}(I)) \ne q$ or~$\sigma(q+1) \notin I$ for any~$I \in \II$. Since~$|p-q| > 1$, it implies that~$\min(\tau^{-1}(I)) \ne q$ or~$\tau(q+1) \notin I$. Hence we obtain that~$\Or_\tau = \Or_{\tau'}$.
%
%\para{If~$p = q-1$.} Then~$\sigma = X i j k Y$, $\sigma' = X j i k Y$ and $\tau = X i k j Y$ for some letters~$1 \le i < j < k \le n$ and some words~$X,Y$ on~$[n]$. Let $\sigma'' \eqdef X j k i Y$, $\tau' \eqdef X k i j Y$ and~$\tau'' \eqdef X k j i Y$.
%If~$\Or_\tau \ne \Or_{\tau'}$, then there exist~$1 \le u \le i < k \le v \le n$ such that~$[u,v] \in \II$ and~$p = \min \big( \tau^{-1}([u,v]) \big)$. 
%This would imply that~$p = \min \big( \sigma^{-1}([u,v]) \big)$, which contradicts that~$\Or_\sigma = \Or_{\sigma'}$.
%If~$\Or_{\tau'} \ne \Or_{\tau''}$, then there exists~$1 \le u \le i < j \le v < k$ such that~$[u,v] \in \II$ and~$q = \min \big( \tau'^{-1}([u,v]) \big)$. 
%This would imply that~$q = \big( \sigma^{-1}([u,v]) \big)$, contradicting that~$\Or_\sigma = \Or_{\sigma'}$.
%Since~$\sigma' < \tau''$ and~$\sigma < \sigma'$, we obtain by induction that~$\projUp(\sigma') \le \projUp(\tau'')$.
%As~$\Or_\sigma = \Or_{\sigma'}$ and~$\Or_\sigma = \Or_{\tau'} = \Or_{\tau''}$, we conclude that~$\projUp(\sigma) \le \projUp(\tau)$.
%
%\para{If~$p = q+1$.} Then~$\sigma = X i j k Y$, $\sigma' = X i k j Y$ and $\tau = X j i k Y$ for some letters~${1 \le i < j < k \le n}$ and some words~$X,Y$ on~$[n]$.
%Let~$\sigma'' \eqdef X k i j Y$, $\tau' \eqdef X j k i Y$ and~$\tau'' \eqdef X k j i Y$. 
%If~$\Or_\tau \ne \Or_{\tau'}$, then there exist~$1 \le u \le i < k \le v \le n$ such that~$[u,v] \in \II$ and~$p = \min \big( \tau^{-1}([u,v]) \big)$.
%As~$\II$ is closed under final subintervals, we have~$[j,v] \in \II$ and~$p = \min \big( \sigma^{-1}([j,v]) \big)$, which contradicts that~$\Or_\sigma = \Or_{\sigma'}$.
%If~$\Or_{\tau'} \ne \Or_{\tau''}$, then there exists~$1 \le u \le j < k \le v \le n$ such that~$[u,v] \in \II$ and~$q = \min \big( \tau'^{-1}([u,v]) \big)$. 
%This would imply that~$p = \big( \sigma^{-1}([u,v]) \big)$, contradicting that~$\Or_\sigma = \Or_{\sigma'}$.
%Since~$\sigma' < \tau''$ and~$\sigma < \sigma'$, we obtain by induction that~$\projUp(\sigma') \le \projUp(\tau'')$.
%As~$\Or_\sigma = \Or_{\sigma'}$ and ${\Or_\tau = \Or_{\tau'} = \Or_{\tau''}}$, we conclude that~$\projUp(\sigma) \le \projUp(\tau)$.
%\end{proof}

\begin{proof}
We only prove the meet semilattice morphism, the result for join semilattice morphism follows by the symmetry of~\cref{prop:antiIsomorphism}.
...
If~$\tau = \projDown(\tau)$, we have~$\projDown(\sigma) \le \sigma < \tau = \projDown(\tau)$. % or~$\projDown(\sigma) = \projDown(\tau)$.
...
\begin{corollary}
For an interval hypergraph $\II$ on~$[n]$ with~$\{i\} \in \II$ for all~$i \in [n]$, the map~$\Or$ from the weak order on~$\fS_n$ to $P_\II$ is a lattice morphism if and only if $\II$ is closed under subintervals.
In this case, $P_\II$ is a Cartesian product of Tamari lattices.
\end{corollary}

%\vincent{Conversely, the quotients that appear as interval hypergraphic polytopes are those whose ideal of arcs is closed under~$(i,j,A,B) \mapsto (i,j,A \ssm \{k\}, B \cup \{k\})$ for~$i < j < k$ and~$A \sqcup B = {]i,j[}$.}
