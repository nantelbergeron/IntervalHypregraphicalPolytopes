...
\begin{example}
\label{ex:DH2}
Using $\HH=\{ 1, 2, 3, 4, 123, 134 \}$ as in~\cref{ex:DH1}, we order the two non-singleton $(123,134)$.
...
$\Or_{4132} = (1, 4, 3, 4)$ is an acyclic orientation of~$\II$ (for the lexicographic order $(123, 1234, 23, 234)$ of the non-singletons).
We will represent interval hypergraphs and their orientations graphically as follows:
\[
	\II =  \raisebox{-.2cm}{\intervalHypergraph{4}{1/3,1/4,2/3,2/4}}
	\qquad\qquad
	\Or_{4132}  =  \raisebox{-.2cm}{\acyclicOrientation{4}{1/3/1,1/4/4,2/3/3,2/4/4}}
\]
As before, we omit to draw the singletons $\{i\}$ for $i \in [n]$.
See also~\cref{fig:exmInterval}.
%
\begin{figure}
	\centerline{
		\begin{tabular}[t]{c}
			\input{figures/hypergraphicalPolytope} \\[.2cm]
...
\begin{notation}
\label{notation:muj}
For~$j \in \cJ_\II$,  we let
$$ J_j=\!\!\! \bigcap_{j \in I \ssm \{\min(I)\}} \!\! I,$$
and set~$\mu_j \eqdef \min(J_j)$ and~$\nu_j \eqdef \max(J_j)$.
%be the inclusion minimal set in~$\smash{\II_j \eqdef \bigset{I \in \II}{j \in I \ssm \{\min(I)\}}}$ ($J_j$ exists since $\II$ is closed under intersection) 
%$\mu_j \eqdef \min(J_j) = \max\bigset{\min(I)}{I \in \II_j}$ and~$\nu_j \eqdef \max(J_j) = \min\bigset{\max(I)}{I \in \II_j}$.
...
$$\displaystyle J_{ij} =  \bigcap_{\{i,j\}\subseteq I}  I$$ and set~$\mu_{ij} \eqdef \min(J_{ij})$ and~$\nu_{ij} \eqdef \max(J_{ij})$.
%denote by~$J_{ij}$ the inclusion minimal set in~$\smash{\II_{ij} \eqdef \bigset{I \in \II}{\{i,j\} \subseteq I}}$ ($J_{ij}$ exists since $\II$ is closed under intersection) and set~$\mu_{ij} \eqdef \min(J_{ij}) = \max\bigset{\min(I)}{I \in \II_{ij}}$ and~$\nu_{ij} \eqdef \max(J_{ij}) = \min\bigset{\max(I)}{I \in \II_{ij}}$.
By definition~$\mu_{ij} \le i < j \le \nu_{ij}$ since~$\{i,j\} \subseteq J_{ij}$.
\end{notation}
...
\begin{remark}
In~\cref{notation:muj}, for $j\in \cJ_\II$ we constructed $J_j$ and $\mu_j$. The set 
$$X_j={[\mu_{j}, j[} \; \ssm \!\!\!\! \bigcup_{\substack{J \in \II \\ J \subseteq {[\mu_{j}, j[}}} \!\!\!\!\big( J \ssm \{\min(J)\}\big)$$
 is nonempty as it contains at least $\mu_j$. Let $\mu_i\le i_j=\max X_j$. We have that 
 $$ \mu_{i_jj}=\mu_j\qquad\text{ and } J_{i_jj}=J_j.$$
 Furthermore, the pair $(i_j,j)\in\cIJ_\II$ since it satisfies all the conditions of the definition. Comparing~\cref{not:Aj} and~\cref{not:joinIrreducibles} we see that $A_j=A_{i_jj}$.
 Moreover, for all $(k,j)\in \cIJ_\II$, we must have $k\le i_j$. See~\cref{ex:AjAij} through~\cref{ex:lastAij} below.
\end{remark}
