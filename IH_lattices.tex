...
In view of these two examples, we would like to characterize the hypergraphs~$\HH$ for which $P_\HH$ is a lattice, a distributive lattice, a semidistributive lattice, a congruence-uniform lattice, a \mbox{(semi-)lattice} quotient of the weak order on permutations.
These questions were settled in~\cite{Pilaud-acyclicReorientationLattices} for graphical zonotopes (\ie when~$\HH \subseteq \binom{[n]}{2}$), and also partially studied in~\cite{BarnardMcConville} for graph associahedra~\cite{CarrDevadoss} (\ie when~$\HH$ is the set of all subsets of vertices that induce a connected subgraph of a fixed graph on~$[n]$).
...
By symmetry,\nantel{Should we specify which symmetry (i.e. refer to~\cref{prop:antiIsomorphism})}
suppose~$O(I_1) < O(I_2)$.
...
\vincent{recall basic definitions and properties of semidistributive lattices. Canonical join representations.}
\end{definition}
...
\vincent{todo}
\vincent{Note that we cannot use interval doubling since some interval hypergraph lattices are join semidistributive but not meet semidistributive or the opposite.}
\vincent{Is it possible to use the BEZ type lemma for semidistributivity? (See Reading's chapter). This means that we only have to check the join semidistributivity condition when $y$ and $z$ are covered by a common element.}
\vincent{Another method is to prove that for any cover relation $x \lessdot y$ of~$L$, the set $K_\join(x \lessdot y) \eqdef \set{z \in L}{x \join z = y}$ admits a unique minimal element~$k_\join(x \lessdot y)$. Then this also gives the canonical join representation for free.}
...
First, we can assume that~$\II$ is closed under intersection, otherwise $P_\II$ is not even a lattice by \cref{prop:latticeForward}.
Assume now that there is~$[a,a'], [b,b'], [c,c'], [d,d'] \in \II$ such that $a < b \le a' < b'$, $a < c \le b' < c'$, $d < \min(b, c)$ and~$b' < d'$, and there is no~$[e,e'] \in \II$ such that~$e < b$ and~$b' < e' < c'$.
Let
\[
j \eqdef \max \Big( [d,c'[ \ssm \!\!\!\!\! \bigcup_{\substack{J \in \II \\ J \subseteq [d,c'[}} \!\!\!\!\! J \ssm \{\min(J)\} \Big)
\quad\text{and}\quad
i \eqdef \mu_{jc'} \eqdef \max\set{\min(I)}{\{j,c'\} \subseteq I \in \II}.
\]
Note that $i \le j < b < b' < c'$.
...
\begin{align*}
\pi_X & \eqdef 1 \cdots (i-1) \, j \, i \cdots (j-1) \, (j+1) \cdots (b-1) \, c' \, b' \, b \cdots (b'-1) \, (b'+1) \cdots (c'-1) \, (c'+1) \cdots n \\
\pi_Y & \eqdef 1 \cdots (i-1) \, b' \, i \cdots (b'-1) \, (b'+1) \cdots n \\
\pi_Z & \eqdef 1 \cdots (i-1) \, c' \, i \cdots (c'-1) \, (c'+1) \cdots n
\end{align*}
(written in one line notation), and let~$X \eqdef \Or_{\pi_X}$, $Y \eqdef \Or_{\pi_Y}$ and $Z \eqdef \Or_{\pi_Z}$ be the corresponding acyclic orientations of~$\II$.

\para{Claim 1:} $X \join Y = X \join Z$.
%
Note that~$\pi_X = \projDown_X$.
Indeed, $\pi_X$ has only $3$ descents $ji$, $c'b'$ and~$b'b$.
Let~$\tau$, $\sigma$ and $\rho$ denote the permutations obtained from~$\pi_X$ by exchanging $ji$, $c'b'$ and $b'b$, respectively.
Then
\[
\Or_{\tau}(J_{jc'}) = i \ne j = X(J_{jc'}),
\;\;
...
\Or_{\rho}([b,b']) = b \ne b' = X([b,b']).
\]
Hence, $\pi_X$ is indeed the minimal permutation for~$X$.
Similarly, $\pi_Y = \projDown_Y$ and $\pi_Z = \projDown_Z$.

We therefore obtain from \cref{prop:latticeBackward} that
$X \join Y = \Or_{\pi_X \join \pi_Y}$ and $X \join Z = \Or_{\pi_X \join \pi_Z}$.
%\[
%X \join Y = \Or_{\pi_X \join \pi_Y}
%\qquad\text{and}\qquad
%X \join Z = \Or_{\pi_X \join \pi_Z}.
%\]
As
\begin{align*}
\pi_X \join \pi_Y & = 1 \cdots (i-1) \, c' \, b' \, j \, i \cdots (j-1) \, (j+1) \cdots (b'-1) \, (b'+1) \cdots (c'-1) \, (c'+1) \cdots n \\
\pi_X \join \pi_Z & = 1 \cdots (i-1) \, c' \, j \, i \cdots (j-1) \, (j+1) \cdots (b-1) \, b' \, b \cdots (b'-1) \, (b'+1) \cdots (c'-1) \, (c'+1) \cdots n
\end{align*}
%(still in one line notation), 
and there is no~$[e,e'] \in \II$ such that~$e < b$ and~$b' < e' < c'$ by assumption, we have~$X \join Y = X \join Z$.

\para{Claim 2:} $X \ge Y \meet Z$.
%
For any interval~$I$, we have
\[
Y(I) = \begin{cases} b' & \text{if $b' \in I$ and~$i \le \min(I)$,} \\ \min(I) & \text{otherwise} \end{cases}
\quad\text{and}\quad
Z(I) = \begin{cases} c' & \text{if $c' \in I$ and~$i \le \min(I)$,} \\ \min(I) & \text{otherwise} \end{cases}
\]
Hence, $(Y \meet Z)(I) = \min(I)$ except maybe if~$[b',c'] \subseteq I$ and~$i \le \min(I)$.
For such~$I$, we actually have that
\begin{itemize}
\item $(Y \meet Z)(I) \le j$ since $Y(I) = b'$ and~$Z(J) = \min(J)$ for all~$J \subseteq [d,c'[$ and using the definition of~$j$,
\item $X(I) = j$ since~$j \in I$ and~$i \le \min(I)$.
\end{itemize}
Hence, we obtain that~$X(I) \ge (Y \meet Z)(I)$ for all~$I \in \II$ so that~$X \ge Y \meet Z$.

\para{Claim 3:} $X \join Y = X \join Z \ne X = X \join (Y \meet Z)$.
%
The first equality holds by Claim 1.
The last equality holds by Claim 2.
The inequality holds since~$X(J_{jc'}) = j$ while~$(X \meet Z)(J_{jc'}) \ge Z(J_{jc'}) = c'$.
...
and
\[
X = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,3/4/4,1/4/1}}
\qquad
Y = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,3/4/3,1/4/3}}
\qquad
Z = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/2,3/4/4,1/4/4}}
\]
so that
\[
X \join Y = X \join Z = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,3/4/4,1/4/1}} \ne X = X \join \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/2,3/4/3,1/4/1}} = X \join (Y \meet Z).
\]
\end{example}
...
and
\[
X = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,2/4/4,1/4/1}}
\qquad
Y = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,2/4/3,1/4/3}}
\qquad
Z = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/2,2/4/4,1/4/4}}
\]
so that
\[
X \join Y = X \join Z = \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/2,2/4/2,1/4/1}} \ne X = X \join \raisebox{-.2cm}{\acyclicOrientation{4}{1/2/1,2/3/3,2/4/4,1/4/1}} = X \join (Y \meet Z).
\]
\end{example}
