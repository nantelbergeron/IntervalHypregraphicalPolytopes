...
\begin{example}
\vincent{todo}
The hypergraphic poset of the hypergraph containing all singletons and all initial intervals is a lattice (in fact, it is the boolean lattice).
The hypergraphic poset of the hypergraph containing all intervals is a lattice (in fact, it is the Tamari lattice).
\end{example}
...
\begin{example}
\vincent{todo: one more concrete example.}
The hypergraphic poset of the hypergraph containing all singletons and all initial intervals is a distributive lattice (in fact, it is the boolean lattice).
...
\[
	...
	\foreach \x in {1,...,3}
		\node (\x) at (\x*.5,-.1) [inner sep = -1pt] {$\scriptstyle \x$};
	\draw [thick,{Bar[width=3pt]}-{Bar[width=3pt]}] (.5,.4)--(1,.4);   \node at (.5,.4) {$\bullet$};
	\draw [thick,{Bar[width=3pt]}-{Bar[width=3pt]}] (1,.2)--(1.5,.2);   \node at (1.5,.2) {$\bullet$};
	\draw [thick,{Bar[width=3pt]}-{Bar[width=3pt]}] (.5,.6)--(1.5,.6);   \node at (1.5,.6) {$\bullet$};
	\end{tikzpicture}
\]
\end{remark}
...
If an interval hypergraph~$\II$ is not distributive, there is a join irreducible acyclic orientation~$A$ such that~$A \not\le A_j$ for all~$j \in \cJ_\II$.
\end{lemma}

\begin{proof}
As $\II$ is not distributive, there exist~$I,J,K \in \II$ with~$I \not\subseteq J$, $I \not\supseteq J$, $I \cap J \ne \varnothing$, and~$I \cap J$ is neither initial nor final in~$K$.
Assume by symmetry that~$\min(I) < \min(J) \le \max(I) < \max(J)$, and let~$j \eqdef \min(J \ssm I)$ and~$k \eqdef \min(K)$.
Let~$A \eqdef \Or_{(k, \dots, j)}$ be the acyclic orientation of~$\II$ obtained as the image by the surjection map~$\Or$ of \cref{def:surjection} of the cycle permutation~$(k, \dots, j)  = 12 \cdots (k-1)j\, k \cdots (j-1) (j+1) \cdots n$.
In other words,
\[
A(K) = 
\begin{cases}
j & \text{if } j \in J \text{ and } \min(J) \ge k \\
\min(J) & \text{otherwise.}
\end{cases}
\]
We claim that the flip of~$j$ to~$k$ is the only decreasing flip from~$A$.
\vincent{todo}
