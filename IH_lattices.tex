...
\begin{example} On the right side of~\cref{Fig:Orientation1}, we find the poset $P_\HH$ associated to 
$\Delta_\HH$ for $\HH=\{ 1, 2, 3, 4, 123, 134 \}$. 
\nantel{done}
\end{example}
...
\begin{example} Continuing with the example $\HH=\{ 1, 2, 3, 4, 123, 134 \}$, let $A$ be the acyclic orientation $(2,4)$.
The order~$\less_A$ is the following
\[
	\less_A= \text{Trans. cl. }\Big(
	...
	\end{tikzpicture} 	
\]
There are four linear extensions of this order that correspond to the permutations $2413$, $2431$, $4213$ and $4231$.
\nantel{done}
\end{example}
...
\Nantel{if Felix proof holds, we will have to add a remark stating the general statement and citiong a forthcoming paper}
...
\Nantel{Since we are not using the following section anymore, it is very technical and no so relevant anymore... I vote to remove it}
