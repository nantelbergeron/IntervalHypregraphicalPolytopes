...
Consider a join semidistributive interval hypergraph~$\II$.
Consider a cover relation~$\flip{A}{p}{q}{B}$ of~$P_\II$ and assume by means of contradiction that there are~$(i,j), (k,\ell) \in \cIJ_\II$ such that~$A_{ij}$ and~$A_{k\ell}$ are both minimal in~$P_\II$ with~$A \join A_{ij} = B = A \join A_{k\ell}$.
...
From the observation above, we have~$A(T) \ge j$, as otherwise~$j \in [A(T), A(S)] \subseteq T$ and~$A(T) < j$ which would contradict the observation above.
...
We have thus found~$R \eqdef [r,r']$, $S \eqdef [s,s']$, $T \eqdef [t,t']$ and~$U \eqdef [u,u']$ in~$\II$ with~$r < s \le r' < s'$, $r < t \le s' < t'$, and~$u < \min(s,t)$ and~$s' < u'$.
