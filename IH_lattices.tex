...
\begin{proof}
Assume that~$\II$ is a join (resp.~meet) semidistributive interval hypergraph.
Consider a cover relation~$\flip{A}{p}{q}{B}$ of~$P_\II$ and assume by means of contradiction that there are~$(i,j), (k,\ell) \in \cIJ_||$ such that~$A_{ij}$ and~$A_{k\ell}$ are both minimal in~$P_\II$ such that~$A \join A_{ij} = B = A \join A_{k\ell}$.
As~$A_{ij}$ and~$A_{k\ell}$ are incomparable, we have~$j \ne \ell$ by \cref{lem:irrorder2}, and we assume by symmetry that~$j < \ell$.

As~$A_{ij} \not\le A$ but~$A_{ij} \le B$, there is~$U \in \II$ such that~$A(U) < A_{ij}(U) \le B(U)$ by \cref{prop:sourceOrderI}.
Since~$A$ and~$B$ only differ by the flip of~$p$ to~$q$, this implies that~$A(U) = p$ while~$B(U) = q$.
Moreover, by definition of~$A_{ij}$, we obtain that~$\mu_{ij} \le \min(I) \le p < j \le q$.
Similarly, ${\mu_{k\ell} \le p < \ell < q}$.

Observe that for any~$J \in \II$ such that~$A(J) \in {]p,q]}$, we have
\begin{itemize}
\item $p < \min(J)$ as otherwise~$A(I) = p \in J$ while~$A(J) \in {]p,q]} \subseteq I \ssm \{A(I)\}$ would contradict the acyclicity of~$A$,
\item if~$j \in J$, then~$A(J) = B(J) \ge A_{ij}(J) = j$.
\end{itemize}
%Observe that for any~$J \in \II$ such that~$j \in J \subseteq {]p,q[}$, we have~$A(J) = B(J) \ge A_{ij}(J) = j$ (since~$j \in J$ and~$\mu_{ij} \le p < \min(J)$).
Since~$\flip{A}{p}{q}{B}$ is a cover relation of~$P_\II$, \cref{prop:isCoverI} implies that there is~$S \in \II$ such that~$A(S) \in {]p,q]}$ and~$j \in S \ssm \{A(S)\}$.
%Note that~$p < \min(J)$ as otherwise~$A(I) = p \in J$ while~$A(J) \in {]p,q]} \subseteq I$ would contradict the acyclicity of~$A$.
We have~$p < \min(S)$ and we moreover assume at the moment that~$\max(S) < q$.
As~$A(S) \in S \subseteq {]p,q[}$, \cref{prop:isCoverI} implies that there is~$T \in \II$ such that~$A(T) \in {]p,q]}$ and~$A(S) \in T \ssm \{A(T)\}$.
We choose such a~$T$ such that~$\max(T)$ is minimal.
From the observation above, we have~$A(T) \ge j$, as otherwise~$\min(T) \le A(T) < j \le A(S) \le \max(T)$ so that~$j \in T$ which would contradict the observation above.
We deduce that~$A(T) > \max(S)$ as otherwise~$A(T) \in S$ and~$A(S) \in T \ssm \{A(T)\}$ would contradict the acyclicity of~$A$.
As~$\mu_{ij} \le p < \min(S)$ and~$j \in S$, we have~$i < \min(S)$, from which we deduce that there is~$R \in \II$ such that~$R \subseteq {[\mu_{i,j},j [}$ and~$\min(S) \in R \ssm \min(R)$.
Let~$r,r',s,s',t,t',u,u'$ be such that~$[r,r'] \eqdef R$, $[s,s'] \eqdef S$, $[t,t'] \eqdef T$, and~$[u,u'] \eqdef U$.
As~$\min(S) \in R \ssm \min(R)$ and~$\max(R) < j \in S$, we have~$r < s \le r' < s'$.
As~$A(S) \in T$ and~$\max(S) < A(T) \in T$, we have~$t \le s' < t'$.
\vincent{I have a problem here to prove that~$r < t$...}
As~$[p,q] \subseteq U$ and~$p < \min(S)$, $\max(S) < q$, and~$p < \min(T)$, we have~$u < \min(s,t)$ and~$s' < u'$.
As~$\II$ is join semidistributive, there is~$V \eqdef [v,v'] \in \II$ such that~$v < s$ and~$s' < v' < t'$.
As~$\II$ is closed under intersection, we can assume that~$u \le v$.
Hence, if~$v \le p$, then~$A(V) = p$ since otherwise~$A(U) = p \in V \ssm \{A(V)\}$ and~$A(V) \in V \subseteq U$ would contradict the acyclicity of~$A$.
\vincent{I have a problem here to prove that~$B(V) = p$.}
As~$B(V) = p \in J_{ij}$ and~$B(J_{ij}) \ge A_{ij}(J_{ij}) = j \in V \ssm \{B(V)\}$, this contradicts the acyclicity of~$B$.
Hence, we obtain that~$p < v < s \le A(S) \le s' < v < t'$, so that~$V$ contradicts our choice of~$T$.

\vincent{Still need to explain the case~$\max(S) \ge q$.}
\end{proof}
