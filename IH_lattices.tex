...
\newcommand{\cJ}{\mathcal J} % basic irreducibles index
...
In view of Lemma~\ref{lem:coverI}, to get a better characterization of covers, we need to characterize when $B'=(a b)A$ is not a flip (it is not acyclic), and if it is, when is $B'\not< B$.
Proposition~\ref{prop:acyclic} and Proposition~\ref{prop:sourceorder} answers those questions. For the first question, given $A$ and $a$, let
 $${\mathcal A}(a)= \bigcup_{(a,I)\in A} I $$
 $${\mathcal A'}(a)= {\mathcal A}(a)\ssm  \bigcup_{(c,J)\in A\atop c\in {\mathcal A}(a,b)} (J \ssm \{c\})$$

\begin{lemma}\label{lem:isaflip}
 $B=(a,b)A$ is a flip if and only if $b\in {\mathcal A'}(a)$.
\end{lemma}

\Nantel{working this section... not sure yet which direction to take}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Fix $\II$ a segment hypergraph closed under intersections. Let
...
\begin{equation}\label{eq:nu}
 \nu(j)=\min\big\{ \max(I) \big| \ I\in\II, \min(I)=\mu(i)\big\}\,,
\end{equation}
and
\begin{equation}\label{eq:nu}
...
\begin{lemma}\label{lem:irrOj}
	$O_j$ is join irreducible.
\end{lemma}


\begin{proof} $j$ is the only source that is non-minimal in some $I\in \II$ and they all have the same $\min(I)=\mu(j)$.
hence we can only do a single flip down of $j$. \nantel{we need description of cover}
\end{proof}
...
Thanks!
\Nantel{Still to do: Cover, description of $\join$ and $\meet$ in term of sources, quotient(?)}
